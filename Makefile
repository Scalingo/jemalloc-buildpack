...
console:
	...
	@docker run --pull=always --rm -ti -v $(shell pwd):/buildpack -e "STACK=scalingo-22" -w /buildpack scalingo/scalingo:22 \
...
scalingo-22: src/jemalloc-$(VERSION).tar.bz2
	docker run --pull=always --rm -it --volume="$(ROOT_DIR):/wrk" \
		scalingo/scalingo-22:latest /wrk/build.sh $(VERSION) scalingo-22

# Build for scalingo-24 stack
...
build-scalingo-22:
	$(MAKE) scalingo-22 VERSION=3.6.0
	...
	$(MAKE) scalingo-22 VERSION=4.5.0
